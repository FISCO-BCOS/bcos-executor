...
struct SubState {
  ...
  protocol::LogEntriesPtr logs =
      std::make_shared<protocol::LogEntries>(); ///< Any logs.
...
struct CallParameters {
  ...
  CallParameters(std::string_view _senderAddress, std::string_view _codeAddress,
                 std::string_view _receiveAddress, int64_t _gas,
                 bytesConstRef _data, bool _staticCall)
      : senderAddress(_senderAddress), codeAddress(_codeAddress),
        receiveAddress(_receiveAddress), gas(_gas), data(_data),
...
struct EVMSchedule {
  ...
  EVMSchedule(bool _efcd, bool _hdc, unsigned const &_txCreateGas)
      : tierStepGas(std::array<unsigned, 8>{{0, 2, 3, 5, 8, 10, 20, 0}}),
        exceptionalFailedCodeDeposit(_efcd), haveDelegateCall(_hdc),
...
struct ImportRequirements {
  ...
  enum {
    ...
    TransactionSignatures =
        32,            ///< Check the basic structure of the transactions.
    Parent = 64,       ///< Check parent block header
    PostGenesis = 256, ///< Require block to be non-genesis.
    CheckTransactions =
        TransactionBasic | TransactionSignatures, ///< Check transaction
                                                  ///< signatures.
    OutOfOrderChecks =
        ValidSeal | CheckTransactions, ///< Do all checks that can be done
 ...
 */
inline evmc_address
toEvmC(const std::string_view &_addr) { // TODO: add another interfaces for wasm
  evmc_address ret;
