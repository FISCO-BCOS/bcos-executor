...
namespace {
evmc_status_code
transactionStatusToEvmcStatus(protocol::TransactionStatus ex) noexcept {
  switch (ex) {
...
u256 TransactionExecutive::gasLeft() const { return m_remainGas; }

void TransactionExecutive::initialize(Transaction::ConstPtr _transaction) {
  m_transaction = _transaction;

  m_baseGasRequired =
      (m_transaction->type() == protocol::TransactionType::ContractCreation)
  ...
  if (m_baseGasRequired > (bigint)txGasLimit) {
    ...
    BOOST_THROW_EXCEPTION(OutOfGasLimit() << RequirementError(
                              (bigint)(m_baseGasRequired), (bigint)txGasLimit));
  }
}
...
bool TransactionExecutive::execute(bool _staticCall) {
  ...
  if (txGasLimit < m_baseGasRequired) {
    ...
    m_exceptionReason =
        ...
        boost::lexical_cast<std::string>(txGasLimit) +
        " require: " + boost::lexical_cast<std::string>(m_baseGasRequired);
    BOOST_THROW_EXCEPTION(OutOfGasLimit() << errinfo_comment(
                              "Not enough gas, base gas required:" +
                              std::to_string(m_baseGasRequired)));
  }
  ...
  if (m_transaction->type() == TransactionType::ContractCreation) {
    ...
    return call(string(m_transaction->to()), string(m_transaction->sender()),
                m_transaction->input(), txGasLimit - m_baseGasRequired,
                _staticCall);
  }
}
...
std::string TransactionExecutive::newEVMAddress(const std::string_view &_sender,
                                                bytesConstRef _init,
                                                u256 const &_salt) {
  auto hash = m_hashImpl->hash(bytes{0xff} + toBytes(_sender) +
                               toBigEndian(_salt) + m_hashImpl->hash(_init));
  return string((char *)hash.data(), 20);
}

bool TransactionExecutive::create(const std::string_view &_txSender,
                                  int64_t _gas, bytesConstRef _init,
...
bool TransactionExecutive::createOpcode(const std::string_view &_sender,
                                        int64_t _gas, bytesConstRef _init,
                                        const std::string_view &_origin) {
...
bool TransactionExecutive::create2Opcode(const std::string_view &_sender,
                                         int64_t _gas, bytesConstRef _init,
                                         const std::string_view &_origin,
                                         u256 const &_salt) {
...
bool TransactionExecutive::call(const std::string &_receiveAddress,
                                const std::string &_senderAddress,
                                bytesConstRef _data, int64_t _gas,
                                bool _staticCall) {
  if (m_blockContext->isWasm()) {
    CallParameters params{_senderAddress, _receiveAddress, _receiveAddress,
  ...
  CallParameters params{_senderAddress, receiveAddress, receiveAddress,
                        _gas,           _data,          _staticCall};
  return call(params, _senderAddress);
}

bool TransactionExecutive::call(CallParameters const &_p,
  ...
  if (false) {
    EXECUTIVE_LOG(DEBUG) << LOG_DESC(
                                "execute transaction failed for account frozen")
  ...
  if (m_blockContext &&
      m_blockContext->isEthereumPrecompiled(precompiledAddress)) {
  ...
  } else if (m_blockContext &&
             m_blockContext->isPrecompiled(precompiledAddress)) {
    try {
      ...
      m_output =
          owning_bytes_ref{std::move(callResult->m_execResult), 0, outputSize};
      ...
      if (false) {
    EXECUTIVE_LOG(DEBUG) << LOG_DESC(
                                "execute transaction failed for ContractFrozen")
    ...
    m_excepted = TransactionStatus::ContractFrozen;
      ...
      if (true) {
    auto c = m_context->code();
    h256 codeHash = m_context->codeHash();
    m_context = make_shared<HostContext>(
        m_blockContext, m_contextID, _p.receiveAddress, _p.senderAddress,
...
bool TransactionExecutive::executeCreate(const std::string_view &_sender,
                                         const std::string_view &_origin,
                                         const std::string &_newAddress,
                                         int64_t _gasLeft, bytesConstRef _init,
                                         bytesConstRef constructorParams) {
  //   if (m_s->frozen(_origin)) {
  //     EXECUTIVE_LOG(DEBUG) << LOG_DESC(
  //                                 "deploy contract failed for account
  //                                 frozen")
  //                          << LOG_KV("account", _origin);
  //     writeErrInfoToOutput("Frozen account:0x" + string(_origin));
  //     m_remainGas = 0;
  ...
  // We can allow for the reverted state (i.e. that with which m_context is
  // constructed) to contain the m_orig.address, since we delete it explicitly
  // if we decide we need to revert.

  m_remainGas = _gasLeft;
  //   bool accountAlreadyExist =
  //       (m_s->addressHasCode(_newAddress) || m_s->getNonce(_newAddress) > 0);
  ...
  //     revert();
  //     m_context = {}; // cancel the _init execution if there are any
  //     scheduled. return !m_context;
  //   }
  ...
  if (!_init.empty()) {
    ...
    if (m_blockContext->isWasm()) { // the Wasm deploy use a precompiled which
                                    // call this function, so inject meter here
      if (!hasWasmPreamble(
              *code)) { // if isWASM and the code is not WASM, make it failed
        revert();
        m_context = {};
    ...
    m_context = make_shared<HostContext>(
        m_blockContext, m_contextID, _newAddress, _sender, _origin,
    ...
    try {
      auto getEVMCMessage = [=]() -> shared_ptr<evmc_message> {
        ...
        if (m_remainGas > int64max ||
            m_context->getBlockContext()->gasLimit() > int64max) {
          EXECUTIVE_LOG(ERROR)
        ...
        return shared_ptr<evmc_message>(new evmc_message{
            kind, flags, static_cast<int32_t>(m_context->depth()), leftGas,
            toEvmC(m_context->myAddress()),
            (uint8_t *)m_context->myAddress().data(),
            (int32_t)m_context->myAddress().size(), toEvmC(m_context->caller()),
            (uint8_t *)m_context->caller().data(),
            (int32_t)m_context->caller().size(), m_context->data().data(),
            m_context->data().size(), toEvmC(h256(0)), toEvmC(0x0_cppui256)});
      };

      auto code = m_context->code();
      ...
      if (m_isCreation) {
        ...
        auto ret = vm->exec(m_context, mode, evmcMessage.get(), code.data(),
                            code.size());
        parseEVMCResult(ret);
        ...
        if (outputRef.size() > m_context->evmSchedule().maxCodeSize) {
          m_exceptionReason =
              "Code is too log: " +
              boost::lexical_cast<std::string>(outputRef.size()) + " limit: " +
              boost::lexical_cast<std::string>(
                  m_context->evmSchedule().maxCodeSize);
          BOOST_THROW_EXCEPTION(OutOfGas());
        }

        if (outputRef.size() * m_context->evmSchedule().createDataGas >
        ...
        m_remainGas -=
            outputRef.size() * m_context->evmSchedule().createDataGas;

        m_context->setCode(outputRef.toBytes());
      } else {
        auto mode = toRevision(m_context->evmSchedule());
        auto evmcMessage = getEVMCMessage();
        auto ret = vm->exec(m_context, mode, evmcMessage.get(), code.data(),
...
evmc_result
TransactionExecutive::waitReturnValue(Error::Ptr e,
                                      protocol::ExecutionResult::Ptr result) {
  ...
  m_waitResult = [&prom, callCreate = m_callCreate](
                     bytes &&output, int32_t status, int64_t gasLeft,
                     std::string_view newAddress) {
    evmc_result result;
    result.status_code =
        transactionStatusToEvmcStatus((protocol::TransactionStatus)status);
...
bool TransactionExecutive::continueExecution(bytes &&output, int32_t status,
                                             int64_t gasLeft,
                                             std::string_view newAddress) {
  if (m_waitResult) {
...
bool TransactionExecutive::finalize() {
  ...
  if (m_context)
    m_context->sub().refunds += m_context->evmSchedule().suicideRefundGas *
                                m_context->sub().suicides.size();
...
void TransactionExecutive::parseEVMCResult(std::shared_ptr<Result> _result) {
  ...
  default: {
    if (_result->status() <= EVMC_INTERNAL_ERROR) {
      BOOST_THROW_EXCEPTION(InternalVMError{}
                            << errinfo_evmcStatusCode(_result->status()));
    } else { // These cases aren't really internal errors, just more specific
...
void TransactionExecutive::loggingException() {
  if (m_excepted != TransactionStatus::None) {
    EXECUTIVE_LOG(ERROR) << LOG_BADGE("TxExeError")
                         ...
                         << LOG_KV("TransactionExceptionID",
                                   (uint32_t)m_excepted)
                         << LOG_KV("hash",
                                   m_transaction->signatureData().empty()
                                       ? "call"
