...
using returnCallback = std::function<void(
    bcos::Error::Ptr &&, bcos::protocol::ExecutionResult::Ptr &&)>;
} // namespace executor
...
public:
  ...
  /// environment info.
  TransactionExecutive(const std::shared_ptr<BlockContext> &_blockContext,
                       int64_t _contextID, unsigned _level = 0)
      : m_blockContext(_blockContext), m_contextID(_contextID),
        m_hashImpl(_blockContext->hashHandler()), m_depth(_level),
        m_gasInjector(
  ...
  void setReturnCallback(returnCallback _callback) {
    m_returnCallback = _callback;
  }
  void callReturnCallback(Error::Ptr e, protocol::ExecutionResult::Ptr result) {
    m_returnCallback(std::move(e), std::move(result));
  }
  bool continueExecution(bytes &&output, int32_t status, int64_t gasLeft,
                         std::string_view newAddress);
  evmc_result waitReturnValue(Error::Ptr e,
  ...
  /// required).
  bool create(const std::string_view &_txSender, int64_t _gas,
              bytesConstRef _code, const std::string_view &_originAddress);
  /// @returns false iff go() must be called (and thus a VM execution in
  /// required).
  bool createOpcode(const std::string_view &_sender, int64_t _gas,
                    bytesConstRef _code,
                    const std::string_view &_originAddress);
  /// @returns false iff go() must be called (and thus a VM execution in
  /// required).
  bool create2Opcode(const std::string_view &_sender, int64_t _gas,
                     bytesConstRef _code,
  ...
  /// @returns false iff go() must be called (and thus a VM execution in
  /// required).
  bool executeCreate(const std::string_view &_txSender,
                     const std::string_view &_originAddress,
                     const std::string &_newAddress, int64_t _gas,
                     bytesConstRef _code,
...
private:
  ...
  void
  updateGas(std::shared_ptr<precompiled::PrecompiledExecResult> _callResult);

  std::shared_ptr<BlockContext>
      m_blockContext; ///< Information on the runtime environment.
  int64_t m_contextID = 0;
  crypto::Hash::Ptr m_hashImpl;
  std::shared_ptr<HostContext>
      m_context;             ///< The VM externality object for the VM execution
  ...
  int64_t m_baseGasRequired; ///< The base amount of gas requried for executing
                             ///< this transaction.
  int64_t m_remainGas =
      0; ///< The gas for EVM code execution. Initial amount before go()
         ///< execution, final amount after go() execution.

  protocol::Transaction::ConstPtr
      m_transaction; ///< The original transaction. Set by setup().
  ...
  std::function<void(bytes &&output, int32_t status, int64_t gasLeft,
                     std::string_view newAddress)>
