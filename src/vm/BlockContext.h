...
public:
  ...
  using ParallelConfigCache =
      tbb::concurrent_map<std::pair<std::string, uint32_t>,
                          std::shared_ptr<bcos::precompiled::ParallelConfig>>;
  BlockContext(std::shared_ptr<storage::StateStorage> _tableFactory,
               crypto::Hash::Ptr _hashImpl,
               const protocol::BlockHeader::ConstPtr &_current,
               protocol::ExecutionResultFactory::Ptr _executionResultFactory,
               const EVMSchedule &_schedule, CallBackFunction _callback,
               bool _isWasm);
  using getTxCriticalsHandler =
      std::function<std::shared_ptr<std::vector<std::string>>(
  ...
  getPrecompiled(const std::string &_address) const;

  void
  setAddress2Precompiled(const std::string &_address,
                         std::shared_ptr<precompiled::Precompiled> precompiled);
  ...
  void setPrecompiledContract(
      std::map<std::string, std::shared_ptr<precompiled::Precompiled>> const
          &precompiledContract);

  void commit();

  std::shared_ptr<storage::StateStorage> getTableFactory() {
  ...
  getTxCriticals(const protocol::Transaction::ConstPtr &_tx) {
    return m_getTxCriticals(_tx);
  }
  void setTxCriticalsHandler(getTxCriticalsHandler _handler) {
    m_getTxCriticals = _handler;
  }
  ...
  /// @return timestamp
  uint64_t timestamp() const { // FIXME: update framework when timestamp() of
  ...
  protocol::BlockHeader::ConstPtr currentBlockHeader() {
  ...
  void insertExecutive(int64_t contextID, std::string_view address,
                       std::shared_ptr<TransactionExecutive>);
  std::shared_ptr<TransactionExecutive>
  getLastExecutiveOf(int64_t contextID, std::string_view address);

  protocol::ExecutionResult::Ptr createExecutionResult(int64_t _contextID,
                                                       CallParameters &_p);
  protocol::ExecutionResult::Ptr
  createExecutionResult(int64_t _contextID, int64_t _gasLeft,
                        bytesConstRef _code, std::optional<u256> _salt);
...
private:
  ...
  u256 m_gasLimit;
  bool m_isWasm = false;
  std::map<std::string, std::shared_ptr<PrecompiledContract>>
